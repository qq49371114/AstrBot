        ...
        try:
            with Timer(f"动作{action_planner_info.action_type}", cycle_timers):
                if action_planner_info.action_type == "complete_talk":
                    ...
                    try:
                        # 从 Planner 的 action_data 中提取未知词语列表和 kb_keywords
                        unknown_words = None
                        kb_keywords = None
                        ...
                        logger.info(f"{self.log_prefix}检测到 kb_keywords={kb_keywords}")
                        if kb_keywords:
                            # ========== 第一次回复：告知用户正在查询 ==========
                            logger.info(f"{self.log_prefix}执行知识库查询，第一次回复（查询中），kb_query_prompt={get_querying_prompt()[:50]}...")

                            # 初始化变量
                            first_reply_text = ""
                            first_loop_info = None
                            first_reply_success = False
                            ...
                            success, llm_response = await generator_api.generate_reply(
                                ...
                                from_plugin=False,
                                kb_query_prompt=get_querying_prompt(),  # 传入"查询中"提示
                            )
                            ...
                            if kb_result.status == KBQueryStatus.COMPLETED:
                                ...
                                success, llm_response = await generator_api.generate_reply(
                                    ...
                                    request_type="replyer",
                                    from_plugin=False,
                                    kb_query_prompt=get_result_prompt(kb_result),  # 传入查询结果
                                )
